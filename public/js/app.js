...
function askConfirm(message,options={}){if(CONFIRM_RESOLVE)return Promise.resolve(false);$('confirmChoiceWrap')?.classList.add('hidden');return new Promise(resolve=>{CONFIRM_RESOLVE=resolve;CONFIRM_FOCUS=document.activeElement;const modal=$('confirmModal');$('confirmTitle').textContent=options.title||'确认操作';$('confirmOkBtn').textContent=options.confirmText||'继续';$('confirmMessage').textContent=String(message).replace(/\\n/g,'\n');modal.dataset.tone=options.tone||'default';openModal('confirmModal','confirmOkBtn');})}
function askChoice({title,message,label,value,options,confirmText='保存'}){if(CONFIRM_RESOLVE)return Promise.resolve(null);const wrap=$('confirmChoiceWrap'),select=$('confirmChoice');$('confirmChoiceLabel').textContent=label;select.innerHTML=options.map(([v,t])=>`<option value="${escAttr(v)}">${esc(t)}</option>`).join('');select.value=value;wrap.classList.remove('hidden');return new Promise(resolve=>{CONFIRM_RESOLVE=ok=>resolve(ok?select.value:null);CONFIRM_FOCUS=document.activeElement;$('confirmTitle').textContent=title;$('confirmOkBtn').textContent=confirmText;$('confirmMessage').textContent=message;$('confirmModal').dataset.tone='default';openModal('confirmModal','confirmChoice')})}
