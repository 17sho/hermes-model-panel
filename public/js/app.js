...
function closeModal(id){const modal=$(id);if(!modal)return;const finish=()=>{modal.classList.remove('is-closing');modal.classList.add('hidden');if(OPEN_MODAL===modal)OPEN_MODAL=null;$('app')?.removeAttribute('inert');$('login')?.removeAttribute('inert');const focus=MODAL_FOCUS;MODAL_FOCUS=null;requestAnimationFrame(()=>focus?.focus?.())};if(window.matchMedia('(prefers-reduced-motion: reduce)').matches)return finish();modal.classList.add('is-closing');modal.querySelector('.modalPanel')?.addEventListener('animationend',finish,{once:true})}
