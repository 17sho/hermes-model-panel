...
async function setProviderPrivateAccess(id,checked,input){
  if(checked&&!await askConfirm('确定允许这个中转站访问内网？\n\n它将能访问 10.x、172.16–31.x、192.168.x、CGNAT 和 IPv6 ULA。云元数据、链路本地和组播地址仍会阻止。')){input.checked=false;return}
  try{const r=await api(`/providers/${id}/private-access`,{method:'PUT',body:JSON.stringify({allowed:checked})});store.state=r.state;renderProviders(store.state.providers);toast(checked?'已允许此中转访问内网':'已关闭此中转的内网访问')}
  catch(e){input.checked=!checked;toast(e.message)}
}
async function addProvider(){try{const body={name:$('addName').value,base_url:$('addUrl').value,api_key:$('addKey').value,api_mode:$('addMode').value,model:$('addModel').value,models:$('addModels').value,allow_private_network:$('addAllowPrivate').checked};const r=await api('/providers',{method:'POST',body:JSON.stringify(body)});store.state=r.state;renderCurrent(store.state.current);renderProviders(store.state.providers);renderCommands(store.state.commands);renderTestTargets(store.state.providers);toast('已添加中转站')}catch(e){toast(e.message)}}
async function fetchModelsForAdd(sourceButton){
  const btn=sourceButton||null;
  const oldText=btn?btn.textContent:'';
  if(btn){btn.disabled=true;btn.textContent='获取中...'}
  try{
    const r=await api('/fetch-models',{method:'POST',body:JSON.stringify({base_url:$('addUrl').value,api_key:$('addKey').value,api_mode:$('addMode').value,allow_private_network:$('addAllowPrivate').checked})});
...
document.addEventListener('change',e=>{const privateToggle=e.target.closest('input[data-action="private-access"]');if(privateToggle){setProviderPrivateAccess(Number(privateToggle.dataset.provider),privateToggle.checked,privateToggle);return}const el=e.target.closest('select[data-action="session-model"]');if(el?.value)setSessionModel(el.dataset.agent,el.dataset.sessionKey,el.value,el)});
loadState();
