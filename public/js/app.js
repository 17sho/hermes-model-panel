...
let PANEL_UPDATE_INFO=null,PANEL_UPDATE_WORKING=false;
function shortSha(sha){return String(sha||'').slice(0,8)||'-'}
function renderPanelRollbacks(items=[]){const box=$('versionRollbackList');if(!box)return;box.innerHTML=items.length?'<div class="versionRollbackTitle">可回滚版本（最多保留2份）</div>'+items.map(x=>`<div class="versionRollbackItem"><span><b>v${esc(x.version)}</b>${esc(shortSha(x.sha))} · ${esc(x.id)}</span><button type="button" data-rollback-id="${esc(x.id)}" data-rollback-version="${esc(x.version)}">回滚</button></div>`).join(''):''}
function setPanelUpdateWorking(working,label='处理中…'){PANEL_UPDATE_WORKING=working;$('versionProgress')?.classList.toggle('hidden',!working);const text=$('versionProgress')?.querySelector('span');if(text)text.textContent=label;$('versionRefreshBtn').disabled=working;$('versionUpdateBtn').disabled=working;document.querySelector('.versionControl')?.classList.toggle('working',working)}
function renderPanelUpdate(r){PANEL_UPDATE_INFO=r;const version='v'+String(r.version||'-');for(const id of ['sidePanelVersion','versionPopoverNumber','panelUpdateVersion']){const el=$(id);if(el)el.textContent=version}const hint=$('versionPopoverHint'),visual=$('versionVisual'),mark=$('versionStatusMark'),dot=$('versionUpdateDot'),run=$('versionUpdateBtn');visual?.classList.toggle('updateAvailable',!!r.update_available);dot?.classList.toggle('hidden',!r.update_available);if(mark)mark.textContent=r.update_available?'!':'✓';if(r.update_available){hint.textContent=`发现新版本 ${shortSha(r.latest_sha)}，当前 ${shortSha(r.installed_sha)}。`;run.classList.remove('hidden')}else{hint.textContent=`已是最新版本 ${shortSha(r.installed_sha||r.latest_sha)}。`;run.classList.add('hidden')}if(r.status?.state==='failed')hint.textContent=`${r.status.operation==='rollback'?'回滚':'更新'}失败：${r.status.message||'未知错误'}`;renderPanelRollbacks(r.rollbacks||[]);const oldHint=$('panelUpdateHint'),oldRun=$('runPanelUpdateBtn');if(oldHint)oldHint.textContent=hint.textContent;if(oldRun)oldRun.classList.toggle('hidden',!r.update_available)}
async function checkPanelUpdate(btn){if(PANEL_UPDATE_WORKING)return;try{btn?.classList.add('loading');const r=await api('/panel-update');renderPanelUpdate(r);return r}catch(e){const hint=$('versionPopoverHint');if(hint)hint.textContent=e.message;toast(e.message)}finally{btn?.classList.remove('loading')}}
async function runPanelUpdate(btn){if(!PANEL_UPDATE_INFO?.latest_sha){await checkPanelUpdate($('versionRefreshBtn'));if(!PANEL_UPDATE_INFO?.latest_sha)return}if(!await askConfirm(`确定更新面板到 ${shortSha(PANEL_UPDATE_INFO.latest_sha)}？\\n更新期间页面会短暂断开，失败会自动回滚。`))return;try{await api('/panel-update',{method:'POST',body:JSON.stringify({expected_sha:PANEL_UPDATE_INFO.latest_sha}),sourceButton:btn});$('versionPopoverHint').textContent='更新已启动，正在下载并验证…';$('versionUpdateBtn').classList.add('hidden');setPanelUpdateWorking(true,'正在下载与安装');setTimeout(waitForPanelUpdate,2500)}catch(e){toast(e.message)}}
async function runPanelRollback(id,version,btn){if(PANEL_UPDATE_WORKING)return;if(!await askConfirm(`确定回滚到 v${version}？\\n当前版本会转为可回滚备份，失败会自动恢复。`))return;try{await api('/panel-rollback',{method:'POST',body:JSON.stringify({id}),sourceButton:btn});$('versionPopoverHint').textContent=`正在回滚到 v${version}…`;setPanelUpdateWorking(true,'正在切换并验证');setTimeout(waitForPanelUpdate,2000)}catch(e){toast(e.message)}}
...
function toggleVersionPopover(force){const pop=$('versionPopover'),btn=$('versionBadgeBtn');if(!pop)return;const show=force===undefined?pop.classList.contains('hidden'):!!force;pop.classList.toggle('hidden',!show);btn?.setAttribute('aria-expanded',String(show));if(show&&!PANEL_UPDATE_INFO)checkPanelUpdate($('versionRefreshBtn'))}
$('versionBadgeBtn')?.addEventListener('click',e=>{e.stopPropagation();toggleVersionPopover()});$('versionRefreshBtn')?.addEventListener('click',()=>checkPanelUpdate($('versionRefreshBtn')));$('versionUpdateBtn')?.addEventListener('click',()=>runPanelUpdate($('versionUpdateBtn')));$('versionRollbackList')?.addEventListener('click',e=>{const btn=e.target.closest('[data-rollback-id]');if(btn)runPanelRollback(btn.dataset.rollbackId,btn.dataset.rollbackVersion,btn)});document.addEventListener('click',e=>{if(!e.target.closest('.versionControl'))toggleVersionPopover(false)});
...
let OPEN_MODAL=null,MODAL_FOCUS=null;
...
function openModal(id,focusId){const modal=$(id);if(!modal)return;MODAL_FOCUS=document.activeElement;OPEN_MODAL=modal;modal.classList.remove('hidden');$('app')?.setAttribute('inert','');$('login')?.setAttribute('inert','');requestAnimationFrame(()=>$(focusId)?.focus?.()||modalFocusable(modal)[0]?.focus())}
function closeModal(id){const modal=$(id);if(!modal)return;modal.classList.add('hidden');if(OPEN_MODAL===modal)OPEN_MODAL=null;$('app')?.removeAttribute('inert');$('login')?.removeAttribute('inert');const focus=MODAL_FOCUS;MODAL_FOCUS=null;requestAnimationFrame(()=>focus?.focus?.())}
...
let MENU_SCROLL_Y=0,MENU_IS_OPEN=false;
