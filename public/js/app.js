...
function showPanel(id){document.querySelectorAll('.panelSection').forEach(el=>{const active=el.id===id;el.classList.toggle('activeSection',active);if(active){el.getAnimations?.().forEach(a=>a.cancel());void el.offsetWidth}});markSideActive(id);try{history.replaceState(null,'','#'+id)}catch{} const scroller=MOBILE_MENU_MQ.matches?window:$('app');scroller?.scrollTo?.({top:0,behavior:window.matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'})}
